...
void assign_patients_to_surgeon(void) {
    ...
    for (int i = 0; i < num_patients; i++) {
        ...
        else if (surgeon[surgeon_index].num_assigned_patients == surgeon[surgeon_index].current_size) {// increase the current_size by 100
            surgeon[surgeon_index].current_size += 100;
...
void free_rooms(void){
    ...
    for (i=0; i<num_rooms; ++i) {
        ...
        // free(room[i].patients_allocated);
    }
    free(room);
}
...
void print_rooms(){
    ...
    for(int i = 0 ; i < num_rooms ; i++){
        printf("%d\t%d\t%d" , room[i].id , room[i].cap, room[i].num_patients_allocated);
        printf("\n");
    }
}
...
void initialize_3d_array() {
    room_schedule = (Patient ****)malloc(num_rooms * sizeof(Patient ***));
    size_of_room_schedule = (int ***)malloc(num_rooms * sizeof(int **));
    
    for (int i = 0; i < num_rooms; i++) {
        room_schedule[i] = (Patient ***)malloc(days * sizeof(Patient **));
        size_of_room_schedule[i] = (int **)malloc(days * sizeof(int *));
        
        for (int j = 0; j < days; j++) {
            room_schedule[i][j] = (Patient **)malloc(10 * sizeof(Patient *)); // Dynamic array for patients
            size_of_room_schedule[i][j] = (int *)malloc(sizeof(int)); // Track size of dynamic array
...
void initialize_room_gender_map(int *room_gender_map, int num_rooms){
    room_gender_map = calloc(num_rooms, sizeof(int));
    // for(int i = 0 ; i<num_rooms ; i++){
    //     for(int j = 0 ; j<2 ; j++)
    //         room_gender_map[i][j] = -1;
    // }
    memset(room_gender_map, -1, num_rooms * sizeof(int));
}
...
GenderRoom *appendGenderRoomNode(GenderRoom *head, GenderRoom *node) {
    if (!head) return node;
    // appendGenderRoomNode(head->next, node);
    GenderRoom *temp = head;
    head = node;
    head->next = temp;
    temp->prev = head;
    return head;
...
void make_3_room_arrays(int *room_gender_map) {
    ...
    // empty_rooms = (int *) calloc (num_empty_rooms, sizeof(int));
    // gender_A_rooms = (int *) calloc (num_gender_A_rooms, sizeof(int));
    // gender_B_rooms = (int *) calloc (num_gender_B_rooms, sizeof(int));
...
int find_max_surgeon_id(Node *head) {
    int max_s_id, i;
    Node *self;
    for (self=head, max_s_id=-1; self; self=self->next)
...
int getPivot_for_OTs(OTs **ot_data_arr, int n, int d) {
    // if the length is more than two then we'll find the avg of 3 random numbers
    // this function will not be invoked if n is 2 or less than 2
    int t1 = n / 4, t2 = (3*n) / 4, t3 = n / 2, i, j, k;
...
int partition_for_OTs(OTs **ot_data_arr, int n, int pivot, int d) {
    int i, j;
    for (i=0, j=n-1; i<=j; ) {
        while (ot_data_arr[i]->max_ot_time[d] >= pivot) ++i;
        while (ot_data_arr[j]->max_ot_time[d] < pivot) --j;
        if (i <= j)
            swap_for_OTs(ot_data_arr, i++, j--);
    }
    return i;
}

void quick_sort_OTs_on_max_ot_time(OTs **ot_data_arr, int n, int d) {
    // Here, we'll sort the OTs array of length - max_surgeon_id + 1 on max_ot_time.
    int pivot, n_left;
    if (n == 1) return;
    if (n == 2) {if (ot_data_arr[0]->max_ot_time[d] < ot_data_arr[1]->max_ot_time[d]) swap_for_OTs(ot_data_arr, 0, 1); return;}
    pivot = getPivot_for_OTs(ot_data_arr, n, d);
    n_left = partition_for_OTs(ot_data_arr, n, pivot , d);
    quick_sort_OTs_on_max_ot_time(ot_data_arr, n_left, d);
    quick_sort_OTs_on_max_ot_time(ot_data_arr+n_left, n-n_left, d);
}

void sort_ot_data_arr(OTs **ot_data_arr, int d) {
    // using Quick sort algorithm to sort OTs on max_ot_time
...
void update_LOS_of_patients(int d) {
    int i, j, g, r_id, p_id, admit_day, release_day, los, days_passed, due_day;
    Patient *patient;
    Node *p, *self;
        ...
        for(self=p; self; self=self->next) {
            ...
            if (admit_day != -1) {
                ...
                if (days_passed >= los) {
                    ...
                    if (room[r_id].num_patients_allocated == 0) {
                        // remove this room from the associated gender array and append it in empty_rooms array
                        if (!g) gen_array = gender_A_rooms;
                        else gen_array = gender_B_rooms;
                        gen_array = appendGenderRoomNode(gen_array, makeGenderRoomNode(&room[r_id]));
                    }
                }
            }
        }
...
int findSuitableRoom(int p_id, GenderRoom *gender_head) {
    ...
    for (self=gender_head; self; self=self->next) {
        ...
        // parse the gender_rooms array and for each room - 
            // find its gender, capacity - num_patients_allocated capacity and whether it is compatible to the patient at hand or not
        r_id = self->pointer->id;
        if (room[r_id].cap == room[r_id].num_patients_allocated) continue;
        ...
        if(flag) continue;
        // allocate this room to this patient as all the conditions satisfy
    ...
    if (patients[p_id].assigned_room_no == -1) {
        ...
        for (self=empty_rooms; self; self=self->next) {
            ...
            gender_head = appendGenderRoomNode(gender_head, makeGenderRoomNode(&room[r_id]));
            // temp = gender_head->next;
            // gender_head = self;
            // gender_head->next = temp;
            // temp->prev = gender_head;
            break;
        }
    }
    /*
    if the occupants / patients of a room have gone, i.e. their length_of_stay is over, then we can shift that room from a gender's room
    array to the empty_rooms_array. We'll calloc memory in the empty_rooms_array and put a -1 as the room_id in the gender's room array.
    By doing so we're being optimal about the selection of rooms and whenever a room is empty we're shifting that room to empty rooms array.
    */
    // return the room allocated
    return r_id;
}
...
void admitFromPQ(PriorityQueue *pq, int d, OTs **ots) {
    HeapNode node;
    int i, j, p_id, delay, s_id, s_duration, ot_id;
    for (i=0; i<pq->current_size; ++i) {
        /*
        void initPQ(PriorityQueue *pq, int initialCapacity);
        void insertNodeInPQ(PriorityQueue *pq, HeapNode node);
        HeapNode extractMaxFromPQ(PriorityQueue *pq);
...
void admit_patients(int *room_gender_map, PriorityQueue *pq) {
    ...
    */
    ...
    int i, j, k, l, d, g, r_id, p_id, s_id, s_duration_sum, max_surgeon_id, len_surgeon_array, temp_patient;
    ...
    for (d=0; d<days; ++d) {
        ...
        update_LOS_of_patients(d);
        //OTs *ot_data_arr[num_ots];
        
        OTs **ot_data_arr = (OTs **)calloc(num_ots, sizeof(OTs *));
        sort_ot_data_arr(ot_data_arr, d);
        OTs *current_ot = ot_data_arr[0];
        /*
        considerations: 
        1. when am i chaning the ot if the current one is full
        2. what arguments to send to the admitFromPQ function?
        */
        admitFromPQ(pq, d, ot_data_arr); // admit patients previously not admitted, i.e. who are in the Priority Queue
        // int current_OT_time = current_ot->max_ot_time[d];
        ...
        for(i=0; i<len_surgeon_array; ++i) {
            if(!s_data_arr[i]->isNull) {
                if (surgeon[s_data_arr[i]->surgeon_id].max_surgery_time[d] == 0 || !surgeon[s_data_arr[i]->surgeon_id].time_left[d]) {
                    // the surgeon is not available and hence add all the patients to the PQ
                    for (k=0; k<s_data_arr[i]->num_assigned_patients; ++k) {
                        temp_patient = s_data_arr[i]->assigned_patients[k];
                        node = makeHeapNode(patients[temp_patient].mandatory, 0, temp_patient);
                ...
                for (j=0; j < s_data_arr[i]->num_assigned_patients; ++j) {
                    ...
                    if (patients[p_id].mandatory) {
                        // assign available OT
                        if (patients[p_id].surgery_duration <= current_ot->time_left[d]) {
                            // assign OT--------------------------------------------------------------------------------------------------
                            patients[p_id].assigned_ot = current_ot->id;
                            // update the time left
                            current_ot->time_left -= patients[p_id].surgery_duration;
                        }
                        // assign available Room
                        // considerations - capacity, gender, compatibility of rooms
                        g = patients[p_id].gen;
                        if (!g) // patient has gender A
                            r_id = findSuitableRoom(p_id, gender_A_rooms);
                        else
                            r_id = findSuitableRoom(p_id, gender_B_rooms);
                    }
                    else {// the patient is optional and hence put him into PQ
                        node = makeHeapNode(patients[p_id].mandatory, 0, p_id);
                        insertNodeInPQ(pq, node);
                    }
                    // if (surgeon[s_id])
                }
...
void initialize_rooms_req(int num_rooms) {
    rooms_requirement = (Rooms_req **)malloc(num_rooms * sizeof(Rooms_req *));
    ...
    for (int i = 0; i < num_rooms; i++) {
        rooms_requirement[i] = (Rooms_req *)malloc(3*days * sizeof(Rooms_req));
...
void main (void) {
    ...
    initialize_room_gender_map(room_gender_map, num_rooms);
