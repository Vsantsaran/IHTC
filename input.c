...
// .......FUNCTIONs ABOVE - PRIORITY QUEUE IMPLEMENTATION FOR KEEPING TRACK OF CARRY FORWARDED PATIENTS..........//

// .............................FUNCTION BELOW IS FOR COMBINING THE TWO LINKED LISTS....................................
...
int partition_for_OTs(OTs **ot_data_arr, int n, int pivot) {
    int i, j;
    for (i=0, j=n-1; i<=j; ) {
        while (ot_data_arr[i]->max_ot_time >= pivot) ++i;
        while (ot_data_arr[j]->max_ot_time < pivot) --j;
        if (i <= j)
...
void quick_sort_OTs_on_max_ot_time(OTs **ot_data_arr, int n, int d) {
    ...
    n_left = partition_for_OTs(ot_data_arr, n, pivot);
...
int findSuitableRoom(int p_id, int *room_gender_map) {
...
void admit_patients(int *room_gender_map, PriorityQueue *pq) {
    ...
    */
    ...
    int i, j, k, l, d, p_id, s_id, s_duration_sum, max_surgeon_id, len_surgeon_array, temp_patient;
            ...
            if(!s_data_arr[i]->isNull) {
                ...
                for (j=0; j < s_data_arr[i]->num_assigned_patients; ++j) {
                    ...
                    if (patients[p_id].mandatory) {
                        ...
                        // considerations - capacity, gender, compatibility, 
                        findSuitableRoom(p_id, room_gender_map);
                    }
