...
int** POPULATION, * G_BEST, CHROMOSOME_SIZE_INTE_GA, * ALL_G_BESTS_STORAGE_PLACE;
...
void populate_room_gender_map(int** room_gender_map) {
    ...
    for (int i = 0; i < num_occupants; ++i) {
        ...
        if (room_id >= 0 && room_id < num_rooms) (*room_gender_map)[room_id] = occupants[i].gen; // Correct indexing
        else printf("Warning: Invalid room_id %d for occupant %d.\n", room_id, i);
    }
}
...
void create_3d_array(void) {
    ...
    put_occupants();
    for (i = 0; i < mandatory_count; ++i) {
        if (patients[i].admission_day == -1 || patients[i].mandatory) continue;
...
void create_json_file(Patient* patients, int n_patients, Nurses* nurse, int num_nurses, int num_rooms, const char* instance_name, const char* output_folder) {
    ...
    int patient_digits = countDigits(n_patients);
...
void reset_valuesInteGA(void) {
    ...
    //patient_structure
    for (int i = 0; i < mandatory_count; i++) {
        mandatory_patients[i]->is_admitted = 0;
        mandatory_patients[i]->assigned_ot = -1;
        mandatory_patients[i]->assigned_room_no = -1;
        mandatory_patients[i]->admission_day = -1;
        free(mandatory_patients[i]->nurses_allotted);
        mandatory_patients[i]->nurses_allotted = NULL;
        mandatory_patients[i]->num_nurses_allotted = 0;
    }
...
int admitPatientsInteGA(int** room_gender_map, int* chromosome) {
    ...
    for (int i = 0; i < mandatory_count; i++) if (max < chromosome[i]) max = chromosome[i];
    // Allocate memory for patient scheduling tracking
    int* unscheduled_mandatory_patients = (int*)calloc(mandatory_count, sizeof(int));
    int* scheduled_mandatory_patients = (int*)calloc(mandatory_count, sizeof(int));

    if (!unscheduled_mandatory_patients || !scheduled_mandatory_patients) {
    ...
    for (int p = 0; p < num_patients; ++p) {
        if (!patients[p].mandatory) continue;
        p_id = chromosome[p];
    ...
    for (i = 0; i < mandatory_count; i++) {
        if (mandatory_patients[i]->admission_day == -1) {
            unscheduled_mandatory++;
            unscheduled_mandatory_patients[i] = 1;
        }
        else {
            scheduled_mandatory++;
            scheduled_mandatory_patients[mandatory_patients[i]->id] = 1;
        }
    }
    free(unscheduled_mandatory_patients);
    free(scheduled_mandatory_patients);
...
void orderCrossoverInteGA(void) {
    ...
    bool* visited1 = (bool*)calloc(mandatory_count, sizeof(bool));
    bool* visited2 = (bool*)calloc(mandatory_count, sizeof(bool));
    ...
    for (i = 0; i < 2; ++i) for (int j = 0; j < 5; ++j) CROSSOVER_OFFSPRING_STORAGE_PLACE[i][mandatory_count + j] = -1;
    ...
    */
    ...
    if ((rand() / (float)RAND_MAX) <= p_m) {
        ...
        if (!patients[MUTATED_OFFSPRING_STORAGE_PLACE[r2]].mandatory) {
            ...
            for (i = r2 + 1; i < mandatory_count; ++i)
                if (patients[MUTATED_OFFSPRING_STORAGE_PLACE[i]].mandatory) {
                    r2 = i;
                    break;
                }
    ...
    for (i = 0; i < 5; ++i) MUTATED_OFFSPRING_STORAGE_PLACE[mandatory_count + i] = -1;
}
...
void insertMutationInteGA(void)
{
    ...
    if (insert_mutation_r1 != -1 && insert_mutation_r2 != -1) {
        ...
        do {
            from = rand() % mandatory_count;
            to = rand() % mandatory_count;
...
void inversionMutationInteGA(void)
{
    ...
    if (inversion_mutation_r1 != -1 && inversion_mutation_r2 != -1) {
        ...
        do {
            start = rand() % mandatory_count;
            end = rand() % mandatory_count;
...
void normalize(void) {
    ...
    */
    ...
    for (i = 0; i < POP_SIZE; ++i) {
        if (POPULATION[i][mandatory_count] > max_violations) max_violations = POPULATION[i][mandatory_count];
        if (POPULATION[i][mandatory_count + 1] > max_cost) max_cost = POPULATION[i][mandatory_count + 1];
    }
    for (i = 0; i < POP_SIZE; ++i) {
        v = (POPULATION[i][mandatory_count] * THREASHOLD) / max_violations;
        c = (POPULATION[i][mandatory_count + 1] * THREASHOLD) / max_cost;
        v_floor = v;
        c_floor = c;
        if ((v_floor + 0.5) < v) v_floor++;
        if ((c_floor + 0.5) < c) c_floor++;
        POPULATION[i][mandatory_count + 2] = v_floor;
        POPULATION[i][mandatory_count + 3] = c_floor;
    }
}
...
void applyIntegratedGA(void)
{
    ...
    for (i = 1; i < POP_SIZE; ++i) {
        if (G_BEST[mandatory_count]) {
            if (POPULATION[i][CHROMOSOME_SIZE_INTE_GA - 1] < G_BEST[CHROMOSOME_SIZE_INTE_GA - 1])
                ...
                if (!POPULATION[i][mandatory_count] || POPULATION[i][mandatory_count] < G_BEST[mandatory_count])
                    memcpy(G_BEST, POPULATION[i], CHROMOSOME_SIZE_INTE_GA * sizeof(int));
            }
        }
        else {
            if (!POPULATION[i][mandatory_count] &&
    ...
    for (CURR_ITER = 0; CURR_ITER < N_GEN; ++CURR_ITER) {
        if ((rand() / (float)RAND_MAX) < p_c) {
            ...
            for (j = 0; j < 2; ++j) {
                ...
                v = (CROSSOVER_OFFSPRING_STORAGE_PLACE[j][mandatory_count] * THREASHOLD) / max_violations;
                c = (CROSSOVER_OFFSPRING_STORAGE_PLACE[j][mandatory_count + 1] * THREASHOLD) / max_cost;
                v_floor = v;
                c_floor = c;
                if ((v_floor + 0.5) < v) v_floor++;
                if ((c_floor + 0.5) < c) c_floor++;
                CROSSOVER_OFFSPRING_STORAGE_PLACE[j][mandatory_count + 2] = v_floor;
                CROSSOVER_OFFSPRING_STORAGE_PLACE[j][mandatory_count + 3] = c_floor;
            ...
            for (j = 0; j < 2; ++j) {
                if (G_BEST[mandatory_count]) {
                    if (CROSSOVER_OFFSPRING_STORAGE_PLACE[j][CHROMOSOME_SIZE_INTE_GA - 1] < G_BEST[CHROMOSOME_SIZE_INTE_GA - 1]) {
                        ...
                        if (!CROSSOVER_OFFSPRING_STORAGE_PLACE[j][mandatory_count] ||
                            CROSSOVER_OFFSPRING_STORAGE_PLACE[j][mandatory_count] < G_BEST[mandatory_count]) {
                            memcpy(G_BEST, CROSSOVER_OFFSPRING_STORAGE_PLACE[j], CHROMOSOME_SIZE_INTE_GA * sizeof(int));
                            last_improvement_iter = CURR_ITER;
                        }
                }
            ...
            if (max_violations == 0) max_violations = 1;
            if (max_cost == 0) max_cost = 1;
            v = (MUTATED_OFFSPRING_STORAGE_PLACE[mandatory_count] * THREASHOLD) / max_violations;
            c = (MUTATED_OFFSPRING_STORAGE_PLACE[mandatory_count + 1] * THREASHOLD) / max_cost;
            v_floor = v;
            c_floor = c;
            if ((v_floor + 0.5) < v) v_floor++;
            if ((c_floor + 0.5) < c) c_floor++;
            MUTATED_OFFSPRING_STORAGE_PLACE[mandatory_count + 2] = v_floor;
            MUTATED_OFFSPRING_STORAGE_PLACE[mandatory_count + 3] = c_floor;
            ...
            if (G_BEST[mandatory_count]) {
                if (MUTATED_OFFSPRING_STORAGE_PLACE[CHROMOSOME_SIZE_INTE_GA - 1] < G_BEST[CHROMOSOME_SIZE_INTE_GA - 1]) {
                    ...
                    if (!MUTATED_OFFSPRING_STORAGE_PLACE[mandatory_count] || MUTATED_OFFSPRING_STORAGE_PLACE[mandatory_count] < G_BEST[mandatory_count]) {
                        memcpy(G_BEST, MUTATED_OFFSPRING_STORAGE_PLACE, CHROMOSOME_SIZE_INTE_GA * sizeof(int));
                        last_improvement_iter = CURR_ITER;
                    }
            }
            else
                if (!MUTATED_OFFSPRING_STORAGE_PLACE[mandatory_count] &&
    ...
    printf("\nViolations in G_BEST: %d", G_BEST[mandatory_count]);
    printf("\nCost of G_BEST: %d\n", G_BEST[mandatory_count + 1]);
    printf("\nNormalized violations in G_BEST: %d", G_BEST[mandatory_count + 2]);
    printf("\nNormalized cost in G_BEST: %d", G_BEST[mandatory_count + 3]);
...
int objectiveFunction(int* chromosome) {
    ...
    return ALPHA * chromosome[mandatory_count + 2] + (1 - ALPHA) * chromosome[mandatory_count + 3];
...
void evaluateViolationsAndCost(int* chromosome) {
    ...
    chromosome[mandatory_count] = nurseAllocationInteGA(); // returns the number of shifts that went without any nurse
    chromosome[mandatory_count] += findViolations();
    chromosome[mandatory_count + 1] = findCost();
}
...
int patient_delay() {
    int cost = 0;
    for (int p = 0; p < num_patients; p++) {
        if (patients[p].admission_day != -1 && patients[p].admission_day > patients[p].surgery_release_day && patients[p].mandatory) {
...
int coc() {
    ...
    for (int p = 0; p < num_patients; p++)
        if (patients[p].admission_day != -1 && patients[p].mandatory)
...
void crossoverElitismInteGA(void)
{
    ...
    if (CROSSOVER_OFFSPRING_STORAGE_PLACE[0][mandatory_count]) {
        if (CROSSOVER_OFFSPRING_STORAGE_PLACE[0][CHROMOSOME_SIZE_INTE_GA - 1] <
            ...
            if (!POPULATION[worst_fitness_chromosome_index][mandatory_count] ||
                CROSSOVER_OFFSPRING_STORAGE_PLACE[0][mandatory_count] < POPULATION[worst_fitness_chromosome_index][mandatory_count])
                memcpy(POPULATION[worst_fitness_chromosome_index], CROSSOVER_OFFSPRING_STORAGE_PLACE[0], CHROMOSOME_SIZE_INTE_GA * sizeof(int));
    }
    else
        if (CROSSOVER_OFFSPRING_STORAGE_PLACE[0][CHROMOSOME_SIZE_INTE_GA - 1] <
    ...
    if (CROSSOVER_OFFSPRING_STORAGE_PLACE[1][mandatory_count]) {
        if (CROSSOVER_OFFSPRING_STORAGE_PLACE[1][CHROMOSOME_SIZE_INTE_GA - 1] <
            ...
            if (!POPULATION[worst_fitness_chromosome_index][mandatory_count] ||
                CROSSOVER_OFFSPRING_STORAGE_PLACE[1][mandatory_count] < POPULATION[worst_fitness_chromosome_index][mandatory_count])
                memcpy(POPULATION[worst_fitness_chromosome_index], CROSSOVER_OFFSPRING_STORAGE_PLACE[1], CHROMOSOME_SIZE_INTE_GA * sizeof(int));
    }
    else
        if (CROSSOVER_OFFSPRING_STORAGE_PLACE[1][CHROMOSOME_SIZE_INTE_GA - 1] <
    ...
    if (CROSSOVER_OFFSPRING_STORAGE_PLACE[0][mandatory_count]) {
        if (CROSSOVER_OFFSPRING_STORAGE_PLACE[0][CHROMOSOME_SIZE_INTE_GA - 1] <
            ...
            if (!POPULATION[second_worst_fitness_chromosome_index][mandatory_count] ||
                CROSSOVER_OFFSPRING_STORAGE_PLACE[0][mandatory_count] < POPULATION[second_worst_fitness_chromosome_index][mandatory_count])
                memcpy(POPULATION[second_worst_fitness_chromosome_index], CROSSOVER_OFFSPRING_STORAGE_PLACE[0], CHROMOSOME_SIZE_INTE_GA * sizeof(int));
    }
    else
        if (CROSSOVER_OFFSPRING_STORAGE_PLACE[0][CHROMOSOME_SIZE_INTE_GA - 1] <
    ...
    if (CROSSOVER_OFFSPRING_STORAGE_PLACE[1][mandatory_count]) {
        if (CROSSOVER_OFFSPRING_STORAGE_PLACE[1][CHROMOSOME_SIZE_INTE_GA - 1] <
            ...
            if (!POPULATION[second_worst_fitness_chromosome_index][mandatory_count] ||
                CROSSOVER_OFFSPRING_STORAGE_PLACE[1][mandatory_count] < POPULATION[second_worst_fitness_chromosome_index][mandatory_count])
                memcpy(POPULATION[second_worst_fitness_chromosome_index], CROSSOVER_OFFSPRING_STORAGE_PLACE[1], CHROMOSOME_SIZE_INTE_GA * sizeof(int));
    }
    else
        if (CROSSOVER_OFFSPRING_STORAGE_PLACE[1][CHROMOSOME_SIZE_INTE_GA - 1] <
...
void mutationElitismInteGA(void)
{
    ...
    if (MUTATED_OFFSPRING_STORAGE_PLACE[mandatory_count]) {
        if (MUTATED_OFFSPRING_STORAGE_PLACE[CHROMOSOME_SIZE_INTE_GA - 1] <
            ...
            if (!POPULATION[worst_fitness_chromosome_index][mandatory_count] ||
                MUTATED_OFFSPRING_STORAGE_PLACE[mandatory_count] < POPULATION[worst_fitness_chromosome_index][mandatory_count])
                memcpy(POPULATION[worst_fitness_chromosome_index], MUTATED_OFFSPRING_STORAGE_PLACE, CHROMOSOME_SIZE_INTE_GA * sizeof(int));
    }
    else
        if (MUTATED_OFFSPRING_STORAGE_PLACE[CHROMOSOME_SIZE_INTE_GA - 1] <
...
{
    int i, j;
    for (j = 0; j < mandatory_count; ++j) if (mandatory_patients[i]->mandatory) POPULATION[0][j] = patients[j].id;
...
void initDataStructuresInteGA(void)
{
    ...
    ALL_G_BESTS_STORAGE_PLACE = (int*)calloc(POP_SIZE, sizeof(int));
    ASSERT(ALL_G_BESTS_STORAGE_PLACE, "Dynamic Memory Allocation Error for ALL_G_BESTS_STORAGE_PLACE");
}
...
-of <output_filename>");
    ...
    */
    int N_MANDATORY_GA_RUNS = 10;
    
    for (i = 0; i < N_MANDATORY_GA_RUNS; i++) {
        applyIntegratedGA();
        reset_valuesInteGA();
        for (j = 0; j < POP_SIZE; ++j) if (!POPULATION[j][mandatory_count]) ALL_G_BESTS_STORAGE_PLACE[j] = 1;
        /*EDITING -
        * First run this algorithm  on the test instances.
        * Check on an average how many only mandatory G_BESTs are coming in the population.
        * According to that number, we'll decide how many iterations do we need to run.
        * We will run the only_mandatory GA multiple times and get X number of violation free chormosomes. (X: to be decided)
        * Then we'll append optional patients at the end of each chromosome and run our GA / hill climbing.
        */
    }
    //algo_end = clock();
