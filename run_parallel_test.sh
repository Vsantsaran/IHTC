...
EXEC="./oanga"                    # executable to create
INPUT_DIR="../data/test"         # input folder
OUTPUT_DIR="output/oanga/test" # solution folder
LOG_DIR="logs/oanga"           # logs folder
TOTAL_RUNS=10                     # number of input files
...
gcc -O3 -march=native -mtune=native -fopenmp oanga.c cJSON.o -o oanga -lm
...
for ((i=1; i<=TOTAL_RUNS; i++)); do
    ...
    INPUT_FILE="${INPUT_DIR}/test${num}.json"
    OUTPUT_FILE="${OUTPUT_DIR}/sol_test_ga${num}.json"
...
echo "🔍 Check running jobs with: ps -u $USER | grep oanga"
