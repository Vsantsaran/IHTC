...
int is_admitted[1000] = {0};
int num_gender_A_rooms, num_gender_B_rooms, num_empty_rooms;
int *gender_A_rooms, *gender_B_rooms, *empty_rooms;
...
void initialize_room_gender_map(int *room_gender_map, int num_rooms){
     room_gender_map = malloc(num_rooms * sizeof(int));
    // for(int i = 0 ; i<num_rooms ; i++){
    //     for(int j = 0 ; j<2 ; j++)
...
void make_3_room_arrays(int *room_gender_map) {
    ...
    for (i=0; i<num_rooms; ++i) {
        if (room_gender_map[i] == -1) {
            ++num_empty_rooms;
            empty_rooms = (int *) realloc (empty_rooms, num_empty_rooms * sizeof(int));
        }
        if (room_gender_map[i] == 1) {
            ++num_gender_B_rooms;
            gender_B_rooms = (int *) realloc (gender_B_rooms, num_gender_B_rooms * sizeof(int));
        }
        else {
            ++num_gender_A_rooms;
            gender_A_rooms = (int *) realloc (gender_A_rooms, num_gender_A_rooms * sizeof(int));
        }
    }
}
...
void update_LOS_of_patients(int *room_gender_map, int d, PriorityQueue *pq) {
    // to be made
}

int findSuitableRoom(int p_id, int *room_gender_map, int *gender_rooms, int n_rooms) {
    // assign available Room and return the room_id
    // considerations - capacity, gender, compatibility
    ...
    // 
    int flag, i, j, r_id, g = patients[p_id].gen;
...
void admit_patients(int *room_gender_map, PriorityQueue *pq) {
    ...
    */
    ...
    for (d=0; d<days; ++d) {
        ...
        update_LOS_of_patients(room_gender_map, d, pq);
        //OTs *ot_data_arr[num_ots];
        
        OTs **ot_data_arr = (OTs **)malloc(num_ots * sizeof(OTs *));
                ...
                for (j=0; j < s_data_arr[i]->num_assigned_patients; ++j) {
                    ...
                    if (patients[p_id].mandatory) {
                        ...
                        if (!g) // patient has gender A
                            r_id = findSuitableRoom(p_id, room_gender_map, gender_A_rooms, num_gender_A_rooms);
                        else
                            r_id = findSuitableRoom(p_id, room_gender_map, gender_B_rooms, num_gender_B_rooms);
                    }
